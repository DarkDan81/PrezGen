...
## 18. Редактор Тем Stage 8 (PPTX-safe)

- [ ] Открыть `/themes` и выбрать существующую тему
- [ ] Проверить, что в редакторе доступны только токены `color/typography/spacing/chart/table/decor` (без структурных layout-параметров)
- [ ] Изменить `logoEnabled/logoText/logoAnchor/logoSize/logoOpacity` и убедиться, что шильдик в превью обновляется сразу
- [ ] Изменить `shapePreset` (`both/left-line/triangle/blob/none`) и убедиться, что декоративные примитивы включаются/выключаются без сдвига контента
- [ ] Изменить `decor.intensity/safeZoneAlpha/titleMultiplier/contentMultiplier` и проверить мгновенный апдейт golden scenes
- [ ] Проверить switch `Compare`: в режиме сравнения одновременно отображаются `Before` и `After`
- [ ] Переключить сцену (`Title/Content/Table/Chart/Cards`) и убедиться, что превью прокручивается к соответствующему слайду
